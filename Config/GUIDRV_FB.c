...
static void _SetPixelIndex(GUI_DEVICE * pDevice, int x, int y, int PixelIndex) {
    ...
    //
      ...
      #define yPhys y
    #endif
    //printf("_SetPixelIndex x=%d,y=%d,PixelIndex=%d\n",x,y,PixelIndex);
    ...
    GUI_USE_PARA(PixelIndex);
    {
      ...
      // TBD by customer...
      //
      long int location = x + y * g_xres;
      pVideoBuffer[location]=PixelIndex;
...
static unsigned int _GetPixelIndex(GUI_DEVICE * pDevice, int x, int y) {
  unsigned int PixelIndex;
    ...
    GUI_USE_PARA(y);
    {
      ...
      //
      long int location = x + y * g_xres;
      PixelIndex=pVideoBuffer[location];
    }
    #if (LCD_MIRROR_X == 0) && (LCD_MIRROR_Y == 0) && (LCD_SWAP_XY == 0)
      #undef xPhys
      #undef yPhys
...
static void _FillRect(GUI_DEVICE * pDevice, int x0, int y0, int x1, int y1) {
  ...
  if (GUI_pContext->DrawMode & LCD_DRAWMODE_XOR) {
    uint16_t IndexMask  = pDevice->pColorConvAPI->pfGetIndexMask();
    for (; y0 <= y1; y0++) {

      long int location = x0 + y0 * g_xres;
      GUI__memset16(&pVideoBuffer[location], PixelIndex ^ IndexMask, (x1-x0+1));
    }
  } else {
    for (; y0 <= y1; y0++) {

      long int location = x0 + y0 * g_xres;
      GUI__memset16(&pVideoBuffer[location], PixelIndex, (x1-x0+1));
    }
  }
  //ioctl(fd, FBIOPAN_DISPLAY, &var);
}
...
static void _DrawBitLine1BPP(GUI_DEVICE * pDevice, int x, int y, U8 const GUI_UNI_PTR * p, int Diff, int xsize, const LCD_PIXELINDEX * pTrans) {
  ...
  case 0:
    do {
      location = x + y * g_xres;
      pVideoBuffer[location]=(*p & (0x80 >> Diff)) ? Index1 : Index0;
      x++;
  ...
  case LCD_DRAWMODE_XOR:
    ...
    do {
      if (*p & (0x80 >> Diff)) {
          location = x + y * g_xres;
          Pixel=pVideoBuffer[location];
          pVideoBuffer[location]=Pixel ^ IndexMask;
  ...
  case 0:
    if (pTrans) {
      for (; xsize > 0; xsize--, x++, p++) {
        Pixel = *p;
        location = x + y * g_xres;
        pVideoBuffer[location]=*(pTrans + Pixel);
        // _SetPixelIndex(pDevice, x, y, *(pTrans + Pixel));
      }
    } else {
      memcpy(pVideoBuffer,p,xsize);
    }
    break;
  case LCD_DRAWMODE_TRANS:
    if (pTrans) {
      ...
      location = x + y * g_xres;
      memcpy(&pVideoBuffer[location],p,xsize);
    }
    break;
  }
  //ioctl(fd, FBIOPAN_DISPLAY, &var);
...
static void _DrawBitLine16BPP(GUI_DEVICE * pDevice, int x, int y, U16 const GUI_UNI_PTR * p, int xsize) {
  long int location = x + y * g_xres;
  memcpy(&pVideoBuffer[location],p,xsize*2);
  //ioctl(fd, FBIOPAN_DISPLAY, &var);
}
/*********************************************************************
*
...
static void _DrawBitLine32BPP(GUI_DEVICE * pDevice, int x, int y, U32 const GUI_UNI_PTR * p, int xsize) {
  long int location = x + y * g_xres;
  memcpy(&pVideoBuffer[location],p,xsize*4);
  // //ioctl(fd, FBIOPAN_DISPLAY, &var);
}
/*********************************************************************
*
